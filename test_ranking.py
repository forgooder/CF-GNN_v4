...
import numpy as np
import dgl
from utils.score_utils import SCORE_MODE_CHOICES, forward_for_score
...
def get_rank(neg_links):
    ...
    if head_target_id != 10000:
        ...
        head_scores = forward_for_score(model_, data, params_.score_mode).squeeze(1).detach().numpy()
    ...
    if tail_target_id != 10000:
        ...
        tail_scores = forward_for_score(model_, data, params_.score_mode).squeeze(1).detach().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--score_mode", type=str, choices=SCORE_MODE_CHOICES, default='original',
                        help="Which model score to use for ranking evaluation")
