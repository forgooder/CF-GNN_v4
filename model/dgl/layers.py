...
class RGCNLayer(nn.Module):
    ...
    def propagate(self, g, attn_rel_emb=None, edge_mask=None):
        raise NotImplementedError

    def forward(self, g, attn_rel_emb=None, edge_mask=None):

        self.propagate(g, attn_rel_emb, edge_mask)
...
class RGCNBasisLayer(RGCNLayer):
    ...
    def propagate(self, g, attn_rel_emb=None, edge_mask=None):
        ...
        edge_weight = self.edge_dropout(torch.ones(g.number_of_edges(), 1).to(weight.device))
        if edge_mask is not None:
            if edge_mask.dim() == 1:
                edge_mask = edge_mask.unsqueeze(1)
            if edge_mask.dim() != 2 or edge_mask.shape[1] != 1:
                raise ValueError(f"edge_mask must have shape [num_edges] or [num_edges, 1], got {tuple(edge_mask.shape)}")
            if edge_mask.shape[0] != g.number_of_edges():
                raise ValueError(f"edge_mask has {edge_mask.shape[0]} rows, expected {g.number_of_edges()}")
            edge_weight = edge_weight * edge_mask.to(device=weight.device, dtype=edge_weight.dtype)
        ...
        had_edge_weight = 'w' in g.edata
        previous_edge_weight = g.edata['w'] if had_edge_weight else None
        g.edata['w'] = edge_weight
        try:
            g.update_all(msg_func, self.aggregator, None)
        finally:
            if had_edge_weight:
                g.edata['w'] = previous_edge_weight
            else:
                g.edata.pop('w')
